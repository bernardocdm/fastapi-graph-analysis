...
from src.config import RAW_DATA_DIR, GITHUB_TOKEN, DEFAULT_REPO

class GitHubMiner:
    """Responsável por coletar dados do repositório GitHub e gerenciar o cache local."""

    def __init__(self, token=None, repo_name=DEFAULT_REPO):
        self.token = token or GITHUB_TOKEN
        self.repo_name = repo_name
        self.cache_file = RAW_DATA_DIR / f"{self.repo_name.replace('/', '_')}_data.json"
        
        if self.token:
            print("[INFO] Autenticado com token fornecido do GitHub.")
    ...
    def mine(self, limit=30, force_refresh=False):
        ...
        Tenta ler do cache primeiro, a menos que force_refresh seja True.
        Se falhar ou não houver credenciais, pode retornar None (para fallback ao mock).
        """
        if not force_refresh and self.cache_file.exists():
            print(f"[INFO] Carregando dados minerados do cache local: {self.cache_file.name}")
            return self.load_cache()

        print(f"[INFO] Iniciando mineração ativa do repositório {self.repo_name} (Limite: {limit} itens)...")
        try:
            ...
            # Coletando issues recentes (inclui PRs na API do GitHub, mas filtramos/separamos)
            print("[INFO] Buscando issues e pull requests...")
            issues = repo.get_issues(state="all")[:limit]
            
            for idx, issue in enumerate(issues):
                print(f"  [{idx+1}/{len(issues)}] Processando #{issue.number}: {issue.title[:40]}...")
                ...
                if is_pull_request:
                    ...
                    try:
                        ...
                        # Em caso de erro ao obter detalhes do PR
                        item_data["reviews"] = []
            ...
            print("[INFO] Mineração concluída com sucesso e dados armazenados em cache.")
            return output_data
